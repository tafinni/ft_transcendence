...
function predictBallYRight() {
  let predictedY = gamedata.ballY;
  let predictedDY = gamedata.ballDY;
  let distanceToPaddle = Math.abs(gamedata.right - gamedata.ballX);

  while (distanceToPaddle > 0) {
    predictedY += predictedDY;
    if (predictedY < -ymax || predictedY > ymax) {
      predictedDY *= -1;
      predictedY += 2 * predictedDY;
    }
    distanceToPaddle -= Math.abs(gamedata.ballDX);
  }
  return predictedY;
}

let timerRight = Date.now() - 500;
let supposedYRight = 0;

function aiMoveRight() {
  let predictedY;
  if (Date.now() - timerRight > 1000) { //"sees" every 1 second
    predictedY = predictBallYRight() - 0.025 / 2; // half of paddle width
    if (predictedY < -ymax) {
      predictedY = -ymax;
    } else if (predictedY + 0.025 / 2 > ymax) {
      predictedY = ymax - 0.025 / 2;
    }
    supposedYRight = predictedY;
    timerRight = Date.now();
  }
  if (gamedata.right < supposedYRight) {
    gamedata.right += Math.min(playerspeed, supposedYRight - gamedata.right);
  } else if (gamedata.right > supposedYRight) {
    gamedata.right -= Math.min(playerspeed, gamedata.right - supposedYRight);
  }
}
  ...
  if (gamedata.ballpass == 0) {
    ...
    if (gamedata.ballpass > ticksToEnd) {
      if (gamedata.ballX < 0)
        ...
        score.left += 1
      serverEnd()
      permanentSocket.emit('score', score)
    }
  }
}
